...
const menubarMenus = ref<MenuGroup[]>([
  ...
  },
  {
    ...
    children: [
      ...
      {
        title: 'editor.emoji.tooltip',
        icon: 'Emoji',
        disabled: () => !props.editor.isEditable,
        action: () => {
          props.editor.chain().toggleEmoji().focus().run()
        },
        requiredExtensions: ['emoji'],
      },
      {
...
<template>
  <Menubar class="rounded-b-none shadow-none border-t-transparent border-l-transparent border-r-transparent rounded-t-[0.5rem]">
    <MenubarMenu
      v-for="(item, index) in loadedMenuItems"
      :key="index"
    >
