...
const menuRef = ref()
const onShortcutChange = ref<boolean>(false)
...
function shortcutClick(item: MenuItem) {
  ...
  prompt.value = (item as ShortcutItem).prompt || ''
  if (onShortcutChange.value) {
    if (!props.editor) {
      toast({
        title: t.value('editor.AI.error'),
        description: t.value('editor.AI.editorNotFound'),
...
watch(
  () => props.editor?.extensionManager.extensions.find(e => e.name === 'AI')?.options?.onShortcutChange,
  newVal => {
    onShortcutChange.value = newVal || false
  },
  { immediate: true }
)

const shortcutMenus = computed<ShortcutItem[]>(() => {
  const options = props.editor?.extensionManager.extensions.find(e => e.name === 'AI')?.options
  const shortcuts: ShortcutItem[] = options?.shortcuts || []
  const hideShortcutDefault: boolean = props.hideShortcutDefault || options?.hideShortcutDefault || false
  let mergedShortcuts = [] as ShortcutItem[]
  if (hideShortcutDefault) {
    mergedShortcuts = shortcuts || []
  } else {
    mergedShortcuts = [...DEFAULT_SHORTCUTS, ...shortcuts]
  }
  // 对菜单项进行本地化处理
...
const inputPaddingClass = computed(() => (selectedLabel.value ? 'pl-24' : 'pl-10'))
</script>
<template>
  <div
    class="absolute left-0 right-0 top-0 bottom-0"
    :style="{
      zIndex: status === 'init' && prompt === '' || status === 'completed' ? -1 : 98,
    }"
    v-show="shouldShow"
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <div
              class="text-sm text-foreground FtContentView"
              :style="{
                padding: 0,
                minHeight: 'auto',
              }"
        ...
        >
          ...
          >
            ...
            <Input
              ...
              class="w-full pr-20 h-12 outline-none ring-0 focus-visible:ring-0 pl-10"
              :style="{ paddingLeft: selectedLabel ? '7.5rem' : '2.5rem' }"
            />
            <!-- 左侧图标 -->
            <span
            ...
            <span
              v-if="selectedLabel"
              class="absolute left-10 top-1/2 -translate-y-1/2 text-xs text-muted-foreground bg-accent px-2 py-0.5 rounded flex items-center"
            ...
            >
              ...
              <Icon
                ...
                @click.stop="selectedLabel = '', store.state.AIMenuShortcut = true"
              />
            </span>
          </div>
          <Button
        ...
        <div
          class="mt-3 max-w-56"
          v-show="status === 'init' && shortcutMenus.length && !prompt && store.state.AIMenuShortcut"
        >
          <Menu
            ref="menuRef"
            :items="shortcutMenus"
